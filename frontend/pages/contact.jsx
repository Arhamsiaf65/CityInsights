...
function Contact() {
  const { contact, user } = useContext(userContext);
  const [message, setMessage] = useState('');

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (user?.name && user?.email && message) {
      const success = await contact(user.name, user.email, message);
      if (success) {
        setMessage('');
      }
    } else {
        ...
        >
          <div>
            ...
            <input
              ...
              value={user?.name || ''}
              readOnly
              className="w-full px-4 py-2 sm:py-3 border border-gray-300 rounded-md bg-gray-100 text-gray-600 focus:outline-none text-sm sm:text-base cursor-not-allowed"
            />
          </div>
          <div>
